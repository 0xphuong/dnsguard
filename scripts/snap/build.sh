...
version="$(./DNSGuard_amd64 --version | cut -d ' ' -f 4)"
if [ "$version" = '' ]; then
	log 'empty version from ./DNSGuard_amd64'
...
for arch in \
	...
	'armhf' \
	'i386'; do
	build_output="./DNSGuard_${arch}"
	snap_output="./DNSGuard_${arch}.snap"
	snap_dir="${snap_output}.dir"

	# Create the meta subdirectory and copy files there.
	mkdir -p "${snap_dir}/meta"
	cp "$build_output" "${snap_dir}/DNSGuard"
	cp './snap/local/dnsguard-web.sh' "$snap_dir"
	cp -r './snap/gui' "${snap_dir}/meta/"

	# Set required permissions.
	chmod o+rx "${snap_dir}/dnsguard-web.sh"
	chmod o+rx "${snap_dir}/meta/gui"
	chmod o+r "${snap_dir}/meta/gui/dnsguard-web.desktop"
	chmod o+r "${snap_dir}/meta/gui/dnsguard-web.png"
