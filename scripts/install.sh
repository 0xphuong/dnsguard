	...
	*)
	...
	for cmd in $required; do
		...
		if ! is_command "$cmd"; then
			...
			error_exit "$cmd is required to install DNSGuard via this script"
		fi
	done
}
...
fix_darwin() {
	...
	# It is important to install DNSGuard into the /Applications directory
...
set_download_func() {
	if is_command 'curl'; then
		...
		error_exit "either curl or wget is required to install DNSGuard via this script"
	fi
}
...
configure() {
	...
	pkg_name="DNSGuard_${os}_${cpu}.${pkg_ext}"
	# This fork publishes packages as GitHub Releases assets rather than on a
	# channel-partitioned CDN, so only the 'release' channel resolves to the
	# latest tag; the others need an explicit tag, which this script does not
	# take yet.
	if [ "$channel" = 'release' ]; then
		url="https://github.com/0xphuong/dnsguard/releases/latest/download/${pkg_name}"
	else
		error_exit "channel '$channel' has no published packages in this fork
only 'release' is available; build from source for the other channels"
	fi
	agh_dir="${out_dir}/DNSGuard"
	readonly pkg_name url agh_dir

	log "DNSGuard will be installed into $agh_dir"
}
...
is_root() {
	...
	if is_command "$sudo_cmd"; then
		log 'note that DNSGuard requires root privileges to install using this script'

		return 1
	fi

	error_exit 'root privileges are required to install DNSGuard using this script
...
rerun_with_root() {
	script_url='https://raw.githubusercontent.com/0xphuong/dnsguard/main/scripts/install.sh'
	readonly script_url
...
handle_existing() {
	...
	if [ "$existing_dnsguard" != '' ]; then
		...
		if [ "$reinstall" -ne '1' ] && [ "$uninstall" -ne '1' ]; then
			error_exit \
				"to reinstall/uninstall the DNSGuard using this script specify one of the '-r' or '-u' flags"
		fi

		# TODO(e.burkov):  Remove the stop once v0.107.1 released.
		if (cd "$agh_dir" && ! ./DNSGuard -s stop || ! ./DNSGuard -s uninstall); then
			...
			log "cannot uninstall DNSGuard from $agh_dir"
		fi

		rm -r "$agh_dir"

		log 'DNSGuard was successfully uninstalled'
	fi
...
install_service() {
	...
	if (cd "$agh_dir" && maybe_sudo ./DNSGuard -s install); then
		return 0
	fi
	...
	error_exit 'cannot install DNSGuard as a service'
}
...
echo 'starting DNSGuard installation script'
...
printf '%s\n' \
	'DNSGuard is now installed and running' \
	'you can control the service status with the following commands:' \
	"$sudo_cmd ${agh_dir}/DNSGuard -s start|stop|restart|status|install|uninstall"
