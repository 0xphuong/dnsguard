...
log 'starting to build DNSGuard release'
...
build() {
	...
	if [ "$build_os" = 'windows' ]; then
		build_output="./${build_dir}/DNSGuard.exe"
	else
		build_output="./${build_dir}/DNSGuard"
	fi
	...
	'darwin' | 'windows')
		...
		# zip.
		(cd "${dist}/${1}" && zip -9 -q -r "../../${build_archive}" "./DNSGuard")
		;;
	*)
		build_archive="./${dist}/${build_ar}.tar.gz"
		tar -C "./${dist}/${1}" -c -f - "./DNSGuard" | gzip -9 - >"$build_archive"
		;;
	esac
...
echo "$platforms" | while read -r os arch arm mips; do
	...
	case "$arch" in
	arm)
		dir="DNSGuard_${os}_${arch}_${arm}"
		ar="DNSGuard_${os}_${arch}v${arm}"
		;;
	mips*)
		dir="DNSGuard_${os}_${arch}_${mips}"
		ar="$dir"
		;;
	*)
		dir="DNSGuard_${os}_${arch}"
		ar="$dir"
		;;
	esac
...
build_archive="./${dist}/DNSGuard_frontend.tar.gz"
...
version_download_url="https://github.com/0xphuong/dnsguard/releases/latest/download"
...
if [ "$channel" = 'edge' ]; then
	announcement_url='https://github.com/0xphuong/dnsguard/wiki/Platforms'
else
	announcement_url="https://github.com/0xphuong/dnsguard/releases/tag/${version}"
fi
...
echo "{
  \"version\": \"${version}\",
  \"announcement\": \"DNSGuard ${version} is now available!\",
...
echo "
  \"download_linux_mips64\": \"${version_download_url}/DNSGuard_linux_mips64_softfloat.tar.gz\",
  \"download_linux_mips64le\": \"${version_download_url}/DNSGuard_linux_mips64le_softfloat.tar.gz\",
  \"download_linux_mipsle\": \"${version_download_url}/DNSGuard_linux_mipsle_softfloat.tar.gz\",
" >>"$version_json"
...
for f in $ar_files; do
	...
	# Remove the prefix.
	platform="${platform#"./${dist}/DNSGuard_"}"
