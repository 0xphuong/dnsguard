# AGENTS.md — DNSGuard client_v2 (SolidJS Frontend)

## Table of Contents

- [AGENTS.md — DNSGuard client_v2 (SolidJS Frontend)](#agentsmd--dnsguard-client_v2-solidjs-frontend)
...
`client_v2` is the next-generation web UI for **DNSGuard**, written in
**SolidJS** and TypeScript. It is a multi-entry single-page application that
runs in the browser and communicates with the DNSGuard backend over its
...
- **Constraints**: Output is embedded into the Go binary's `build/static/`;
  the dev server proxies to a running DNSGuard instance
- **Scale/Scope**: Single-user admin UI (one operator per DNSGuard
  installation)
...
- **Stateless client, stateful backend.** The UI holds no durable state; all
  configuration and data persist in the DNSGuard backend via the `/control`
...
- **E2E tests**: Playwright, configured in `playwright.config.ts`
  ...
  a real DNSGuard backend prepared by `scripts/prepareConfig.mjs`. E2E
