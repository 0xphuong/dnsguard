...
<p align="center">
  <a href="https://github.com/0xphuong/dnsguard">Source</a> |
  <a href="https://github.com/0xphuong/dnsguard/issues">Issues</a>
</p>
<hr/>

> **DNSGuard is a fork of [AdGuard Home][upstream]**, renamed and maintained
> independently by [0xphuong][gh]. It is not affiliated with, endorsed by, or
> supported by AdGuard. Upstream is licensed GPL-3.0 and so is this fork; see
> [LICENSE.txt](LICENSE.txt).
>
> Because this fork has no public release channel yet, there are no build,
> coverage, download-count, or package-store badges here — the upstream ones
> would report AdGuard's numbers, not this project's. Links in this document
> that point at `github.com/AdguardTeam/AdGuardHome`, the AdGuard Knowledge
> Base, or AdGuard's filter lists are deliberate: those resources still belong
> to upstream and are still the correct reference.

[upstream]: https://github.com/AdguardTeam/AdGuardHome
[gh]:       https://github.com/0xphuong

<hr/>

DNSGuard is a network-wide software for blocking ads and tracking. After you set it up, it'll cover ALL your home devices, and you don't need any client-side software for that.

It operates as a DNS server that re-routes tracking domains to a “black hole”, thus preventing your devices from connecting to those servers. It is derived from AdGuard Home, which in turn shares a lot of code with AdGuard's public [AdGuard DNS] servers.
...
- [Getting Started](#getting-started)
    ...
    - [Guides](#guides)
    - [API](#api)
- [Comparing DNSGuard to other solutions](#comparison)
    - [How is this different from public AdGuard DNS servers?](#comparison-adguard-dns)
    - [How does DNSGuard compare to Pi-Hole](#comparison-pi-hole)
    - [How does DNSGuard compare to traditional ad blockers](#comparison-adblock)
...
```sh
curl -s -S -L https://raw.githubusercontent.com/0xphuong/dnsguard/main/scripts/install.sh | sh -s -- -v
```

To install with `wget` run the following command:

```sh
wget --no-verbose -O - https://raw.githubusercontent.com/0xphuong/dnsguard/main/scripts/install.sh | sh -s -- -v
```

To install with `fetch` run the following command:

```sh
fetch -o - https://raw.githubusercontent.com/0xphuong/dnsguard/main/scripts/install.sh | sh -s -- -v
```

The script also accepts some options:

- `-c <channel>` to use specified channel;
- `-r` to reinstall DNSGuard;
- `-u` to uninstall DNSGuard;
...
Please read the **[Getting Started][wiki-start]** article on our Wiki to learn how to install DNSGuard manually, and how to configure your devices to use it.

#### <a href="#docker" id="docker" name="docker">Docker</a>

This fork publishes no image yet. [Docker Hub] hosts the **upstream** AdGuard Home
image, not DNSGuard; to run DNSGuard in Docker, build it yourself — see
[Docker image](#docker-image) below, or use the `docker-compose.yml` in the repo root.

#### <a href="#snap-store" id="snap-store" name="snap-store">Snap Store</a>

The [Snap Store] package is the **upstream** AdGuard Home one. DNSGuard has no
published snap; `snap/` in this repo can build a local `dnsguard` snap.
...
If you want to integrate with DNSGuard, you can use our [REST API][openapi]. Alternatively, you can use this [python client][pyclient], which can be used to build the [DNSGuard Assistant integration][assistant-integration].

[assistant-integration]:   https://www.home-assistant.io/integrations/adguard/
[openapi]:  https://github.com/AdguardTeam/AdGuardHome/tree/master/openapi
[pyclient]: https://pypi.org/project/adguardhome/

## <a href="#comparison" id="comparison" name="comparison">Comparing DNSGuard to other solutions</a>
...
Running your own DNSGuard server allows you to do much more than using a public DNS server. It's a completely different level. See for yourself:
...
### <a href="#comparison-pi-hole" id="comparison-pi-hole" name="comparison-pi-hole">How does DNSGuard compare to Pi-Hole</a>

At this point, DNSGuard has a lot in common with Pi-Hole. Both block ads and trackers using the so-called “DNS sinkholing” method and both allow customizing what's blocked.

> [!NOTE]
> We're not going to stop here. DNS sinkholing is not a bad starting point, but this is just the beginning.

DNSGuard provides a lot of features out-of-the-box with no need to install and configure additional software. We want it to be simple to the point when even casual users can set it up with minimal effort.
...
### <a href="#comparison-adblock" id="comparison-adblock" name="comparison-adblock">How does DNSGuard compare to traditional ad blockers</a>
...
Is there a chance to handle this in the future?  DNS will never be enough to do this. Our only option is to use a content blocking proxy like what we do in the standalone AdGuard applications. We're [going to bring][issue-1228] this feature support to DNSGuard in the future. Unfortunately, even in this case, there still will be cases when this won't be enough or would require quite a complicated configuration.
...
You will need this to build DNSGuard:
...
```sh
git clone https://github.com/0xphuong/dnsguard
cd DNSGuard
make
```
...
You can build DNSGuard for any OS/ARCH that Go supports. In order to do this, specify `GOOS` and `GOARCH` environment variables as macros when running `make`.
...
[Docker Buildx][buildx]. For a single-platform local image, `sh ./docker/prepare-dist.sh`
followed by `docker compose build` is quicker.
...
2. Run your `DNSGuard` binary with the `--local-frontend` flag, which instructs DNSGuard to ignore the built-in frontend files and use those from the `./build/` directory.
...
DNSGuard uses [Playwright](https://playwright.dev) for E2E testing. Tests are located in `tests/e2e`.
...
DNSGuard does not publish prebuilt beta or edge packages. The `beta` and `edge`
channels exist in the build scripts, but nothing is uploaded for them, so
`install.sh -c beta` and `-c edge` deliberately fail rather than silently
fetching upstream AdGuard Home builds.

To run an unstable version, build it from source and pass the channel to `make`:

```sh
make CHANNEL=edge VERBOSE=1 build
```

The Snap Store and Docker Hub `beta`/`edge` artifacts are upstream AdGuard
Home's, not this fork's.
...
DNSGuard has no translation platform of its own; its locale files come from upstream.
To improve the translations everyone inherits, contribute upstream: see AdGuard's
[Knowledge Base][kb-trans] and the [AdGuard Home project on CrowdIn][crowdin].
Locale files in this fork live in `client/src/__locales/` and `client_v2/src/__locales/`.
...
These are third-party projects built for **upstream AdGuard Home**. They are listed
because DNSGuard keeps its API compatible, but none of them target this fork.
...
You might have seen that [CoreDNS] was mentioned here before, but upstream stopped using it in AdGuard Home.
...
The main idea is that you are the one who should be in control of your data. So it is
only natural that DNSGuard does not collect any usage statistics, and does not use any
web services unless you configure it to do so. Upstream's [full privacy policy][privacy]
enumerates every bit that *could in theory be sent*; it still applies to this fork, since
the fork changes only naming, not the data flows.
