...
func TestUpdater_internal(t *testing.T) {
	...
	exePathUnix := filepath.Join(wd, "DNSGuard.exe")
	exePathWindows := filepath.Join(wd, "DNSGuard")
	yamlPath := filepath.Join(wd, "dnsguard.yaml")
	readmePath := filepath.Join(wd, "README.md")
	licensePath := filepath.Join(wd, "LICENSE.txt")

	require.NoError(t, os.WriteFile(exePathUnix, []byte("DNSGuard.exe"), 0o755))
	require.NoError(t, os.WriteFile(exePathWindows, []byte("DNSGuard"), 0o755))
	require.NoError(t, os.WriteFile(yamlPath, []byte("dnsguard.yaml"), 0o644))
	...
	testCases := []struct {
		...
		os:          "linux",
		exeName:     "DNSGuard",
		archiveName: "DNSGuard.tar.gz",
	}, {
		name:        "windows",
		os:          "windows",
		exeName:     "DNSGuard.exe",
		archiveName: "DNSGuard.zip",
	}}
	...
	for _, tc := range testCases {
		...
		require.True(t, t.Run("backup", func(t *testing.T) {
			var d []byte
			d, err = os.ReadFile(filepath.Join(wd, "agh-backup", "dnsguard.yaml"))
			require.NoError(t, err)

			assert.Equal(t, "dnsguard.yaml", string(d))
