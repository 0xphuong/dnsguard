...
func TestUpdater_Update(t *testing.T) {
  ...
  "download_linux_amd64": "%s"
	...
	const packagePath = "/DNSGuard.tar.gz"

	wd := t.TempDir()

	exePath := filepath.Join(wd, "DNSGuard")
	yamlPath := filepath.Join(wd, "dnsguard.yaml")
	readmePath := filepath.Join(wd, "README.md")
	licensePath := filepath.Join(wd, "LICENSE.txt")

	require.NoError(t, os.WriteFile(exePath, []byte("DNSGuard"), 0o755))
	...
	pkgData, err := os.ReadFile("testdata/DNSGuard_unix.tar.gz")
	...
	versionPath := path.Join("/dnsguard", version.ChannelBeta, "version.json")
