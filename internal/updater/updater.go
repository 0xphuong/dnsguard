// Package updater provides an updater for DNSGuard.
package updater
...
// Updater is the DNSGuard updater.
type Updater struct {
	client *http.Client
	logger *slog.Logger
	...
	backupExeName  string // "workDir/agh-backup/DNSGuard[.exe]"
	updateExeName  string // "workDir/agh-update-v0.103.0/DNSGuard[.exe]"
...
// Config is the DNSGuard updater configuration.
type Config struct {
	...
	// Version is the current DNSGuard version.  It must not be empty.
	Version string

	// Channel is the current DNSGuard update channel.  It must be a valid
...
func (u *Updater) prepare(ctx context.Context) (err error) {
	...
	updateExeName := "DNSGuard"
	if u.goos == "windows" {
		updateExeName = "DNSGuard.exe"
	}
...
// version of DNSGuard just downloaded.
func (u *Updater) check(ctx context.Context) (err error) {
	u.logger.InfoContext(ctx, "checking configuration")

	err = copyFile(u.confName, filepath.Join(u.updateDir, "dnsguard.yaml"), aghos.DefaultPermFile)
	if err != nil {
...
func (u *Updater) backup(ctx context.Context, firstRun bool) (err error) {
	...
	if !firstRun {
		err = copyFile(u.confName, filepath.Join(u.backupDir, "dnsguard.yaml"), aghos.DefaultPermFile)
		if err != nil {
			return fmt.Errorf("copyFile() failed: %w", err)
		}
	}
...
// It skips DNSGuard, DNSGuard.exe, and dnsguard.yaml.
func (u *Updater) copySupportingFiles(
	...
	for _, f := range files {
		...
		if name == "DNSGuard" || name == "DNSGuard.exe" || name == "dnsguard.yaml" {
			continue
		}
