...
import (
	...
	"github.com/0xphuong/dnsguard/internal/configmigrate"
	"github.com/0xphuong/dnsguard/internal/next/configmgr"
	"github.com/0xphuong/dnsguard/internal/version"
	"github.com/AdguardTeam/golibs/osutil"
)

// options contains all command-line options for the DNSGuard(.exe) binary.
type options struct {
	...
	//
	//   - "install":  Installs DNSGuard as a system service.
	...
	// checkConfig, if true, instructs DNSGuard to check the configuration
	// file, optionally print an error message to stdout, and exit with a
	// corresponding exit code.
	checkConfig bool

	// disableUpdate, if true, prevents DNSGuard from automatically checking
	// for updates.
	//
	// TODO(a.garipov): Use.
	disableUpdate bool
	...
	// help, if true, instructs DNSGuard to print the command-line option
	// help message and quit with a successful exit-code.
	help bool

	// localFrontend, if true, instructs DNSGuard to use the local frontend
	// directory instead of the files compiled into the binary.
	//
	// TODO(a.garipov): Use.
	localFrontend bool

	// performUpdate, if true, instructs DNSGuard to update the current
	// binary and restart the service in case it's installed.
	//
	// TODO(a.garipov): Use.
	performUpdate bool

	// noPermCheck, if true, instructs DNSGuard to skip checking and
	// migrating the permissions of its security-sensitive files.
	//
	// TODO(e.burkov):  Use.
	noPermCheck bool

	// verbose, if true, instructs DNSGuard to enable verbose logging.
	verbose bool

	// version, if true, instructs DNSGuard to print the version to stdout
...
var commandLineOptions = []*commandLineOption{
	confFileIdx: {
		...
		defaultValue: "internal/next/dnsguard.yaml",
...
// parseOptions parses the command-line options for DNSGuard.
...
// processOptions decides if DNSGuard should exit depending on the results
