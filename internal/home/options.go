...
type options struct {
	...
	// workDir is the path to the working directory where DNSGuard stores
	// filter data, the query log, and other data.
	workDir string

	// logFile is the path to the log file.  If empty, DNSGuard writes to
	...
	// disableUpdate, if set, makes DNSGuard not check for updates.
	disableUpdate bool

	// performUpdate, if set, updates DNSGuard without GUI and exits.
	performUpdate bool
	...
	// localFrontend forces DNSGuard to use the frontend files from disk
...
// cmdLineOpts are all command-line options of DNSGuard.
