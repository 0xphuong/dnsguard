...
type webAPIConfig struct {
	...
	// updater is used for updating DNSGuard.  If disableUpdate is set to
	...
	// firstRun, if true, tells DNSGuard to register install handlers.
	firstRun bool

	// disableUpdate, if true, tells DNSGuard to not check for updates.
	disableUpdate bool
	...
	// serveHTTP3, if true, tells DNSGuard to start HTTP3 server.
	serveHTTP3 bool
}
...
func newWebAPI(ctx context.Context, conf *webAPIConfig) (w *webAPI) {
	...
	if conf.firstRun {
		conf.logger.InfoContext(
			ctx,
			"This is the first launch of DNSGuard, redirecting everything to /install.html",
		)
...
func (web *webAPI) start(ctx context.Context) {
	...
	web.logger.InfoContext(ctx, "DNSGuard is available at the following addresses:")
...
// validatePorts validates the uniqueness of TCP and UDP ports for DNSGuard
// DNS protocols.
func validatePorts(
