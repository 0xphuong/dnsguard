// Package home contains DNSGuard's HTTP API methods.
package home
...
import (
	...
	"github.com/0xphuong/dnsguard/internal/aghnet"
	"github.com/0xphuong/dnsguard/internal/aghos"
	"github.com/0xphuong/dnsguard/internal/aghslog"
	"github.com/0xphuong/dnsguard/internal/aghtls"
	"github.com/0xphuong/dnsguard/internal/arpdb"
	"github.com/0xphuong/dnsguard/internal/dhcpd"
	"github.com/0xphuong/dnsguard/internal/dnsforward"
	"github.com/0xphuong/dnsguard/internal/filtering"
	"github.com/0xphuong/dnsguard/internal/filtering/hashprefix"
	"github.com/0xphuong/dnsguard/internal/filtering/safesearch"
	"github.com/0xphuong/dnsguard/internal/permcheck"
	"github.com/0xphuong/dnsguard/internal/querylog"
	"github.com/0xphuong/dnsguard/internal/stats"
...
func Main(clientBuildFS fs.FS) {
	...
	baseLogger.InfoContext(ctx, "starting dnsguard", "version", version.Full())
	baseLogger.DebugContext(ctx, "current working directory", "path", workDir)
	if opts.runningAsService {
		baseLogger.InfoContext(ctx, "dnsguard is running as a service")
	}
...
func setupContext(
	...
	if isFirstRun {
		baseLogger.InfoContext(ctx, "this is the first time dnsguard has been launched")
...
func suggestedWebPort(ctx context.Context, l *slog.Logger) (p uint16) {
	const webPortEnv = "DNSGUARD_DEFAULT_WEB_PORT"
...
// run configures and starts DNSGuard.  base and sigHdlr must not be nil.
...
// newUpdater creates a new DNSGuard updater.  l and conf must not be nil.
...
func newUpdater(
	...
	const envName = "DNSGUARD_TEST_UPDATE_VERSION_URL"
...
DNSGuard is not allowed to bind to privileged ports (for instance, port 53).
Please note that this is crucial for a server to be able to use privileged ports.

You have two options:
1. Run DNSGuard with root privileges.
...
func checkNetworkPermissions(ctx context.Context, l *slog.Logger) {
	l.InfoContext(ctx, "checking if dnsguard has the necessary permissions")

	if ok, err := aghnet.CanBindPrivilegedPorts(ctx, l); !ok || err != nil {
		l.ErrorContext(
			ctx,
			"this is the first launch of dnsguard; you must run it as administrator.",
		)

		os.Exit(osutil.ExitCodeFailure)
	}

	// We should check if DNSGuard is able to bind to port 53
	...
	l.InfoContext(ctx, "dnsguard can bind to port 53")
}
...
// defaultConfFilename is the default name of the configuration file inside the
// working directory.
const defaultConfFilename = "dnsguard.yaml"

// legacyConfFilename is the configuration file name used before the project was
// renamed from AdGuard Home to DNSGuard.  It is still honored so that an
// installation upgraded in place keeps its settings.
const legacyConfFilename = "AdGuardHome.yaml"

// initConfigFilename returns the configuration file path.  If a path is
// provided via command-line argument, it is used.  Otherwise the default within
// workDir is returned, unless only the legacy file is present, in which case
// that one is used.  l must not be nil.
func initConfigFilename(
	...
	if confPath != "" {
		...
		return legacyFallback(ctx, l, confPath)
	}

	return legacyFallback(ctx, l, filepath.Join(workDir, defaultConfFilename))
}

// legacyFallback returns confPath unchanged, unless it does not exist while a
// file named [legacyConfFilename] does exist next to it, in which case the
// legacy one is returned.  That keeps an installation that predates the rename
// from silently starting over with an empty configuration, including one where
// the path is pinned with -c, as it is in the Docker image.  l must not be nil.
func legacyFallback(ctx context.Context, l *slog.Logger, confPath string) (p string) {
	if _, err := os.Stat(confPath); err == nil {
		return confPath
	}

	legacyPath := filepath.Join(filepath.Dir(confPath), legacyConfFilename)
	...
	l.WarnContext(
		ctx,
		"using legacy config file name; rename it to migrate",
		"path", legacyPath,
		"want", confPath,
	)
...
func cleanup(ctx context.Context, l *slog.Logger, hc *aghnet.HostsContainer) {
	l.InfoContext(ctx, "stopping dnsguard")
...
// detectFirstRun returns true if this is the first run of DNSGuard.  l must
// not be nil.
...
func cmdlineUpdate(
	...
	if err != nil {
		...
		l.InfoContext(ctx, "DNSGuard was not installed as a service. "+
			"Please restart running instances of DNSGuard manually.")
	}
